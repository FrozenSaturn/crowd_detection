...
def extract_color_histogram(frame, bbox, hue_bins=HIST_HUE_BINS, sat_bins=HIST_SAT_BINS):
    """Extracts a 2D H-S color histogram from a bounding box region."""
    x1, y1, x2, y2 = bbox
    if x1 >= x2 or y1 >= y2: return None # Ensure coordinates are valid
...
def calculate_average_group_histogram(frame, group_members_data, hue_bins=HIST_HUE_BINS, sat_bins=HIST_SAT_BINS):
    """Calculates the average H-S color histogram for a group."""
...
def are_persons_close(person1_centroid, person2_centroid, threshold):
    """Checks if two persons are close based on centroid distance."""
...
def calculate_group_centroid(group_members_data):
    """Calculates the average centroid of a group."""
    if not group_members_data: return None
...
def calculate_encompassing_bbox(group_members_data):
    """Calculates the encompassing bounding box for a list of person_data."""
    if not group_members_data: return None
...
def find_potential_groups(persons_data, closeness_threshold, min_persons):
    """Identifies potential groups using connected components based on proximity."""
...
def process_video(video_path, output_path, csv_path):
    ...
    fps = int(cap.get(cv2.CAP_PROP_FPS)) or 25 # Default to 25 FPS if not readable
    ...
    while cap.isOpened():
        ...
        # Handle unmatched active groups (grace period or removal)
        ...
        # 5. Log Confirmed Crowds and Visualize
        for group in active_tracked_groups:
            if group['frames_lost'] > 0: continue
            viz_color = (0,0,255) if group['frames_persisted'] >= CONSECUTIVE_FRAMES_THRESHOLD else (255,165,0)
            ...
            for p_data in group['members']:
                ...
                cv2.rectangle(frame, (x1,y1), (x2,y2), viz_color, 2)
